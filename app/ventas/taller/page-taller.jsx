...
function itemVacio() {
  return { producto_id: '', cantidad: 1, tipo_precio: 'sin_colocacion', precio_unitario: '', busquedaProducto: '' }
}
...
export default function VentasTaller() {
  ...
  const ventasFiltradas = (Array.isArray(ventas) ? ventas : []).filter((v) => {
    ...
    const enProductos = v.items?.some((it) => it.nombre?.toLowerCase().includes(texto))
    const enObservaciones = v.observaciones?.toLowerCase().includes(texto)
    const enId = String(v.id).includes(texto)
    return enProductos || enObservaciones || enId
  })

  return (
    <div className="p-4 sm:p-8">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Ventas - Taller</h1>
          ...
          <div key={index} className="mb-4 pb-4 border-b border-gray-100 last:border-b-0">
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-2">
              <div className="sm:col-span-2">
                ...
                >
                  <option value="">Seleccioná un producto... ({productosFiltradosPara(item).length})</option>
              ...
              <div className="flex gap-2 items-center">
                ...
                <input
                  className="border rounded-lg px-3 py-2 text-base text-gray-800 bg-white w-32"
                  type="number"
                  step="any"
                  placeholder="Precio"
      ...
      <div className="mb-4">
        <div className="relative max-w-md">
          ...
          <input
            type="text"
            placeholder="Buscar por producto, ID u observación..."
            ...
            {ventasFiltradas.length === 0 && (
              <tr>
                <td colSpan={7} className="px-6 py-8 text-center text-gray-400 text-sm">
                  {busquedaHistorial ? `No se encontraron ventas para "${busquedaHistorial}"` : 'No hay ventas registradas todavía'}
                </td>
              </tr>
            )}
          </tbody>
      ...
      {busquedaHistorial && ventasFiltradas.length > 0 && (
        <p className="text-xs text-gray-500 mt-2">{ventasFiltradas.length} de {ventas.length} ventas</p>
      )}
    </div>
  )
}