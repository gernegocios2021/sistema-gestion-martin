...
export function parsearHtmlOP21(html) {
  ...
  for (const fila of filas) {
    ...
    // Los vidrios se agrupan aparte (cortar vs. pedir), no en el mapa global
    if (item.tipo !== "vidrio") acumular(item);
  }

  const materiales = [...mapa.values()];

  // ----- Separación de vidrios: cortar (float simple) vs. pedir a fábrica (DVH) -----
  // Una abertura es DVH si tiene cámara (cmr09) entre sus materiales.
